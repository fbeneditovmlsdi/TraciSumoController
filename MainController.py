...
def run_simutaion(test_type = 0):
    ...
    if test_type == t_type["SPEED_MOV"]:
        with open("ConstBreakLog.txt", "w") as breakLog:
            ...
            car_li = traci._simulation.SimulationDomain.getIDList()
            if isinstance(car_li, (list,)):
                new_car_li = set(car_li) - set(prev_car_li)
            if len(new_car_li) > 0:  # if there is a new car
                prev_car_li = car_li
...
if __name__ == "__main__":
    ...
    if test_type == t_type["PATH"]:
        traci.start([sumoBinary, "-c", "ufma.sumocfg", "fcd-output value", "[PATH]"+output_file_name])
    if test_type == t_type["PUNCTUALITY"]:
        traci.start([sumoBinary, "-c", "ufma_slow.sumocfg", "fcd-output value", "[PUNCTUALITY]"+output_file_name])
    if test_type == t_type["SPEED_MOV"]:
        traci.start([sumoBinary, "-c", "ufma_speed.sumocfg", "fcd-output value", "[SPEED_MOV]"+output_file_name])
    if test_type == t_type["SPEED_STILL"]:
        traci.start([sumoBinary, "-c", "ufma_slow.sumocfg", "fcd-output value", "[SPEED_STILL]"+output_file_name])
